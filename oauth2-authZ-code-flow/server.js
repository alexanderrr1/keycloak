...
app.get('/get/the/code', (req, res) => {
    ...
    const Code_challenge= 'QcjamBUrsrKrfPGEbrPo8x7GPxlyXv52ie0p_iPs05I'
    const Code_challenge_method="S256"
    let url = `${Authorization_Endpoint}?response_type=${Response_Type}&client_id=${Client_Id}&redirect_uri=${Redirect_Uri}&scope=${Scope}&state=${State}&code_challenge=${Code_challenge}&code_challenge_method=${Code_challenge_method}`;
...
app.post('/exchange/the/code/for/a/token', (req, res) => {
    ...
    const Scope = process.env.SCOPE;
    const Code_verifier='4441710393d309aff0b089d776f2eb6d2e481d62b98bc2e73f9ae207'; 
    ...
    let body = `grant_type=${Grant_Type}&code=${Code}&redirect_uri=${encodeURIComponent(Redirect_Uri)}&client_id=${Client_Id}&scope=${encodeURIComponent(Scope)}&code_verifier=${Code_verifier}`;
